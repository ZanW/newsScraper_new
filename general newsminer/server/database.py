...
from server.query import query_tweets
...
class NewinfoSimifre(Base):
    ...
    Tstamp = Column(String, nullable=True)
...
class Database:
    ...
    @classmethod
    def set_raw(cls, temp, H_url, title, news_text, search_job_id):
        ...
        for i in range(len(temp)):
            try:
                ...
                if isinstance(TText, str):
                    ...
                    TText = ""
                sql = "insert into newinfo_raw(Tstamp, Title, URL,search_job_id, Texts) values('" + TmStamp + "','" + TTitle + "','" + HUrl + "','" + str(
                    search_job_id) + "','" + TText + "')"
                conn.query(sql)
                ...
                sql = "insert into newinfo_raw(Tstamp, Title, URL, Texts) values('" + TmStamp + "','" + TTitle + "','" + HUrl + "','" + str(
                    search_job_id) + "','" + TText + "')"
                conn.query(sql)
...
def create_search_job(search_job):
    ...
    if (result != None):
        ...
        session.commit()
        result = get_search_job_in_db(search_job, session)
        search_task(str(search_job.keyword), int(search_job.month), int(search_job.start_date), int(search_job.end_date), int(search_job.limit), int(result.id))
...
def search_task(keyword, month, start_day, end_day, limit, search_job_id):
    ...
    if not title:
        ...
        Database.set_raw(temp, H_url, title, news_text, search_job_id)
