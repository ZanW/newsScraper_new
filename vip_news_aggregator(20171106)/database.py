...
class Database:
    ...
    @classmethod
    def set_raw(cls, v_name, temp, H_url, title, news_text, re_tweet):
        ...
        for i in range(len(temp)):
            try:
                ...
                if isinstance(TText, str):
                    ...
                    TText = ""
                '''
                sql = "insert into vip_news(v_name, time, title, URL, text, fwd_fre) values('" + v_name + "','" + TmStamp + "','" + TTitle + "','" + HUrl + "','" + TText + "','" + Tretweet + "')"
                conn.query(sql)
...
def search_task(V_name, l):
    ...
    if not title:
        ...
        Database.set_raw(V_name, temp, H_url, title, news_text, re_tweet)
