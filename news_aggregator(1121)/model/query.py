...
# set INIT_URL from search_url defined in get_search
def set_init_url(init_url):
    global INIT_URL
    INIT_URL = init_url

'''
print("url is: "+ url)
if Database.set.url == 1:
...
def query_tweets(query, limit=None):
    ...
    while limit is None or len(tweets) < limit:
        ...
        if not new_tweets:
            print("length of raw tweets start in query tweets: " + str(len(tweets)))
            break
